...
10. **Halt-Quality Contract per ticket** (already in run-bolt spec). Make explicit: define DONE criteria, evidence type, regression check. [completion-contracts.md](file:///path/to/project/.claude/rules/completion-contracts.md) is the right pattern; carry it forward into bolt.
